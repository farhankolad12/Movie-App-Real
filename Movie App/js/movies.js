...
function addedBookmarkRemoved() {
  ...
  for (i = 0; i < moviesBody.childElementCount; i++) {
    for (j = 0; j < addedBookmark.length; j++) {
      if (addedBookmark[j] == moviesBody.childNodes[i].innerHTML) {
        moviesBody.childNodes[i].remove();
      }
    }
  }
}
...
const getMovies = async () => {
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&language=en-US&query=${inputValue.value}&page=1`
  );
...
inputValue.addEventListener("input", getMovies);
